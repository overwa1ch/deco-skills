...
description: Designs and reviews executable AI-video cinematography-brief director bodies from supplied scripts, approved storyboards, director scripts, images, audio, notes, or partial materials, then reviews returned videos against the delivered acceptance baseline. Use for 导演设计, 视频导演提示词, 台词与表演设计, Shot 时间线, Shot 内短 beats, 摄影执行, 首帧场景状态与脏乱程度, 光影与色彩连续性, adaptive Audio/BGM/SFX/静音设计, 动作因果, 人物调度, 口型与停顿, targeted edit, extension continuity, prompt review, or 回片审查. Trigger from professional video direction, action, performance, camera, lighting, audio-design, cleaning/product-efficacy setup, or returned-video revision intent. Produce an adaptive body with only task-relevant fields, one consolidated Constraints structure, and top-level Action, Action plus Timing/beats, or Shot blocks with shot-local Action beats according to execution complexity while preserving approved shot authority. Do not create screenplays, storyboards, static assets, Reference bindings, or Route wrappers.
---
...
1. Identify the target video scope, duration, aspect ratio when supplied, locked story facts, dialogue, visual evidence, first-frame inputs, per-Shot initial scene condition, audio or music evidence, required silence, and unresolved conflicts. For cleaning, restoration, organization, damage-removal, before/after, or product-efficacy work, identify the visible contamination or disorder type, amount or density, distribution, and location before action begins; identify an explicit clean and ordered baseline for display Shots.
